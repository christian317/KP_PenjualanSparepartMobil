                    ...
                    <div class="card-body p-0">
                        <table class="table table-hover mb-0" id="tableItem">
                            ...
                            <tbody>
                                <tr class="baris-item">
                                    <td class="ps-4 py-3">
                                        <select name="items[0][id]" class="form-select select2-produk" required>
...
<script>
    ...
    const opsiProdukBersih = `
        <option value="">Ketik Kode / Nama Produk...</option>
        @foreach($semuaProduk as $prod)
            <option value="{{ $prod->id }}">{{ $prod->id }} - {{ $prod->nama }}</option>
        @endforeach
    `;
    ...
    function addBaris() {
        ...
        let rowHTML = `
            <tr class="baris-item">
                <td class="ps-4 py-3">
                    <select name="items[${rowIndex}][id]" class="form-select select2-baru" required>
                        ${opsiProdukBersih}
                    </select>
                </td>
                <td class="py-3 text-center">
