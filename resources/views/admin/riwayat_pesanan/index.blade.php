@extends('layouts.app') {{-- Sesuaikan jika Admin menggunakan layouts.admin --}}

@section('title', 'Riwayat Pesanan Selesai')
@section('page', 'Riwayat Pesanan')
...
@section('content')
    <div class="p-4" style="background-color: #f8f9fa; min-height: 100vh;">
            ...
            <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                <div>
                    ...
                    <div class="text-muted small">
                        Arsip pesanan yang sudah berhasil dikirim maupun dibatalkan.
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card h-100 border-0 shadow-sm border-start border-4 rounded-3"
                    ...
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <div class="text-secondary small fw-semibold">Total Riwayat</div>
            ...
            <div class="col-12 col-md-4">
                <div class="card h-100 border-0 shadow-sm border-start border-4 rounded-3"
                    ...
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-start">
                            <div>
                                <div class="text-secondary small fw-semibold">Pesanan Selesai</div>
        ...
        <form action="{{ url()->current() }}" method="GET"
            ...
            {{-- Hidden Input untuk Tab Navigasi --}}
        ...
        <ul class="nav nav-pills mb-3 bg-white p-2 rounded-3 shadow-sm border overflow-auto flex-nowrap"
            ...
            <li class="nav-item">
                <a class="nav-link {{ request('status') === null || request('status') === '' ? 'active fw-bold' : 'text-muted' }}"
                    ...
                    <i class="bi bi-grid-fill me-1"></i> Semua Riwayat
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ request('status') === '2' ? 'active fw-bold bg-success' : 'text-muted' }}"
                    href="{{ request()->fullUrlWithQuery(['status' => '2', 'page' => 1]) }}">
                    <i class="bi bi-check2-circle me-1"></i> Selesai
                </a>
            </li>
            <li class="nav-item">
                <a class="nav-link {{ request('status') === '3' ? 'active fw-bold bg-danger' : 'text-muted' }}"
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                <div class="modal-content border-0 shadow-lg rounded-4">
                    ...
                    <div class="modal-body p-4 pt-2">
                        <div class="row g-4 mb-4">
                            ...
                            <div class="col-md-6">
                                <div class="p-3 bg-light rounded-3 h-100 border border-light">
                                    ...
                                    <div class="mb-2"><i
                                            class="bi bi-shop text-muted me-2"></i>{{ $p->UserPelanggan->nama_toko ?? '-' }}
                                    </div>
                                    <div class="mb-2"><i
                                            class="bi bi-telephone text-muted me-2"></i>{{ $p->UserPelanggan->telepon ?? '-' }}
                                    </div>
                                    <div><i
                                            class="bi bi-envelope text-muted me-2"></i>{{ $p->UserPelanggan->email ?? '-' }}
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <tr>
                                            <td class="ps-3 py-2">
                                                <div class="fw-semibold text-dark">
                                                    {{ $item->produk->nama ?? 'Produk Tidak Ditemukan' }}</div>
                        ...
                        {{-- INFORMASI PEMBATALAN / REFUND (JIKA ADA) --}}
                        @if ($p->refund || in_array($p->status, [3, 4, 6]))
                            <div class="mt-4">
                                <h6 class="fw-bold text-dark mb-3" style="font-size: 13px;">
                                    <i class="bi bi-arrow-return-left me-2"></i>INFORMASI PEMBATALAN & REFUND
                                </h6>
                                <div class="p-3 bg-danger bg-opacity-10 border border-danger border-opacity-25 rounded-3">
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            @if ($p->status == 6 || ($p->refund && $p->refund->status == 1))
                                                <span class="badge bg-success">Selesai (Refund Berhasil)</span>
                                            @elseif ($p->status == 4 || ($p->refund && $p->refund->status == 0))
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="text-dark small">
                                                {{ $p->refund->alasan_pembatalan ?? 'Dibatalkan oleh sistem / pemilik.' }}
                                            </div>
                                        </div>

                                        {{-- Detail Refund Bank Jika Ada --}}
                                        @if ($p->metode_pembayaran == 0 && $p->refund)
                    ...
                    <div class="modal-footer border-top-0 bg-light py-3 justify-content-end">
