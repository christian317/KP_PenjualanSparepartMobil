...
@section('content')
    <div style="max-width:1100px;margin:0 auto;" class="px-3 px-md-4 py-4 pb-5">
        ...
        <div class="d-flex align-items-center gap-3 mb-4">
            <a href="{{ route('pelanggan.checkout.keranjang') }}"
                                                ...
                                                <div class="d-flex gap-3 align-items-center">
                                                    ...
                                                    <div>
                                                        <div class="fw-semibold small text-truncate"
                                                            style="max-width: 250px;">{{ $item->produk->id }} - {{ $item->produk->nama }}</div>
                            ...
                            <div class="d-flex flex-column gap-3 mb-4">
                                ...
                                <label class="w-100 position-relative m-0 {{ Session::get('status_mitra') != 1 ? 'opacity-50' : 'cursor-pointer' }}"
                                    ...
                                    <input type="radio" name="pay" value="kontrabon" class="position-absolute"
                                        style="opacity: 0; inset: 0; margin: 0; z-index: 10; {{ Session::get('status_mitra') != 1 ? 'cursor: not-allowed;' : 'cursor: pointer;' }}"
                                        {{ Session::get('status_mitra') != 1 ? 'disabled' : '' }}>
    ...
    <style>
        ...
        .radio-custom { width: 18px; height: 18px; border: 2px solid #adb5bd; border-radius: 50%; position: relative; background-color: white; transition: all 0.2s ease; }
        input[type="radio"]:checked + .payment-card { border: 1px solid #dc3545 !important; background-color: rgba(220, 53, 69, 0.05) !important; box-shadow: 0 0 0 3px rgba(220, 53, 69, 0.1); }
        input[type="radio"]:checked + .payment-card .radio-custom { border-color: #dc3545; }
