...
@section('content')
    <div class="container py-4 pb-5 min-vh-100" style="max-width:1240px;">
        ...
        <div class="d-flex gap-2 mb-4 flex-wrap">
            ...
            <button class="btn btn-light border btn-sm rounded-pill px-4 fw-semibold tab-btn" onclick="filterTab(this,'preorder')">
                Pre-Order <span class="badge bg-warning-subtle text-warning-emphasis ms-1">{{ $countPreorder }}</span>
            </button>
            <button class="btn btn-light border btn-sm rounded-pill px-4 fw-semibold tab-btn" onclick="filterTab(this,'kontrabon')">
                Approvel Kontrabon <span class="badge bg-warning-subtle text-warning-emphasis ms-1">{{ $countKontrabon }}</span>
            </button>
            <button class="btn btn-light border btn-sm rounded-pill px-4 fw-semibold tab-btn" onclick="filterTab(this,'refund-dana')">
                Refund Dana <span class="badge bg-warning-subtle text-warning-emphasis ms-1">{{ $countRefundDana }}</span>
            </button>
        ...
        @forelse ($pesanan as $p)
            <div class="card border-0 shadow-sm rounded-4 mb-3 overflow-hidden order-card {{ $p->status == 3 ? 'opacity-75' : '' }}"
                data-status="{{ $p->dataStatus }}">

                <div class="card-header bg-white py-3 border-bottom d-flex flex-wrap justify-content-between align-items-center gap-2">
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <span class="fw-bold text-dark">
                            <i class="bi bi-bag-check me-2 text-danger"></i>{{ $p->nomor }}
                        </span>
                        ...
                        @elseif($p->status == 5)
                            <span class="badge bg-warning bg-opacity-10 text-secondary border border-secondary border-opacity-25 px-2 py-1 rounded-pill">
                                <i class="bi bi-clock me-1"></i>Menunggu Approval Kontrabon
                            </span>
                        @elseif($p->status == 6)
                            <span class="badge bg-warning bg-opacity-10 text-secondary border border-secondary border-opacity-25 px-2 py-1 rounded-pill">
                    ...
                    @elseif ($p->metode_pembayaran == '1')
                        <span class="badge bg-info-subtle text-info-emphasis px-2 py-1 rounded-pill small">
                            <i class="bi bi-file-text me-1"></i>Kontrabon
                        </span>
                    @endif
                </div>
                ...
                {{-- Kontrabon Info (Jatuh Tempo 3 bulan dari tanggal selesai rekapan kontrabon) --}}
                @if ($p->metode_pembayaran == '1' && $p->kontrabon_data->status == 1)
                    <div class="px-4 py-2 border-top bg-info bg-opacity-10 d-flex align-items-center gap-2">
                        <i class="bi bi-info-circle text-info"></i>
                        <span class="small text-info fw-semibold">Kontrabon · Jatuh Tempo:
                            <strong>{{ \Carbon\Carbon::parse($p->kontrabon_data->tanggal_selesai)->addMonths(3)->format('d M Y') }}</strong>
                        </span>
                        @if ($p->status_pembayaran == 1)
                ...
                @if ($p->status == 3)
                    <div class="px-4 py-2 border-top bg-warning bg-opacity-10 d-flex align-items-center gap-2">
                        ...
                        <div>
                            ...
                            <span class="small text-secondary">{{ $p->refund ? $p->refund->alasan_pembatalan : 'Dibatalkan oleh sistem/pengguna.' }}</span>
                        </div>
                    </div>
                @endif
                ...
                <div class="card-footer bg-white border-top py-3 px-4 d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <div>
                        ...
                        <div class="text-muted" style="font-size:11px;">
                            {{ $p->metode_pembayaran == '0' ? 'Cash' : 'Kontrabon' }} · {{ $p->items->count() }} item 
                            @if($p->status == 3 && $p->refund && $p->metode_pembayaran == '0')
                                · Refund Selesai ({{ $p->refund->nama_bank }})
                            @endif
                        </div>
                    </div>
                    
                    <div class="d-flex gap-2 flex-wrap">
                        <a href="{{ route('pelanggan.riwayat.detail_pesanan', ['nomor' => $p->nomor]) }}" class="btn btn-light border btn-sm rounded-3 fw-semibold">
                            <i class="bi bi-eye me-1"></i>Detail Pesanan
                        </a>

                        {{-- Tombol Konfirmasi Diterima --}}
                        @if ($p->status == 1)
                            <form action="{{ route('pelanggan.riwayat.konfirmasi_diterima', $p->nomor) }}" method="POST" class="d-inline">
                                @csrf
            ...
            {{-- MODAL PEMBATALAN PESANAN (CASH & KONTRABON) --}}
            @if (in_array($p->status, [0, 5]))
                <div class="modal fade" id="modalCancel-{{ $p->nomor }}" tabindex="-1">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content border-0 shadow rounded-4 overflow-hidden">
                            <form action="{{ route('pelanggan.riwayat.cancel_pesanan', $p->nomor) }}" method="POST">
                                @csrf
                                <div class="modal-header bg-danger text-white border-0 px-4 py-3">
                                    <div>
                                        <h5 class="modal-title fw-bold mb-0"><i class="bi bi-x-circle me-2"></i>Batalkan & Refund Dana</h5>
                                        <div class="opacity-75 small">{{ $p->nomor }} · Rp {{ number_format($totalHarga, 0, ',', '.') }}</div>
                                    </div>
                                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                </div>
                                <div class="modal-body px-4 py-3">
                                    
                                    {{-- Info Box berbeda untuk Cash dan Kontrabon --}}
                                    ...
                                    @else
                                        <div class="alert border-0 bg-info bg-opacity-10 text-dark d-flex gap-2 align-items-start mb-4 rounded-3">
                                            ...
                                            <div class="small">
                                                Pesanan akan langsung dibatalkan. Tagihan pada Kontrabon Anda otomatis akan dikurangi sesuai dengan nominal pesanan ini.
                                            </div>
                                        </div>
                                    @endif
                                    ...
                                    {{-- Form Bank HANYA MUNCUL JIKA METODE PEMBAYARAN CASH --}}
                                    @if ($p->metode_pembayaran == 0)
                                ...
                                <div class="modal-footer border-0 px-4 py-3 bg-light">
                                    ...
                                    <button type="submit" class="btn btn-danger fw-bold rounded-3 px-4">
                                        <i class="bi bi-check2-all me-2"></i>Konfirmasi Pembatalan
                                    </button>
                                </div>
                            </form>
                        </div>
